...
function alignRight(el) {
  ...
  const top =
    hostOffset.top +
    (DomHandler.getOuterHeight(el) - DomHandler.getOuterHeight(tooltipElement)) / 2;
...
function alignTop(el) {
  ...
  const hostOffset = getHostOffset(el);
  const left =
    hostOffset.left + (DomHandler.getOuterWidth(el) - DomHandler.getOuterWidth(tooltipElement)) / 2;
